...
def train(epoch):
    ...
    for batch_idx, data in enumerate(train_loader):
        ...
        loss = criterion(outputs, targets) #, weights) ## more weight to (5000, 7000)
        loss.backward()
        optimizer.step()
...
def validate(epoch):
    ...
    if current_loss < best_loss:
        ...
        if not os.path.isdir('checkpoint'):
            os.mkdir('checkpoint')
        torch.save(state, './checkpoint/ckptRatio.t7')
        best_loss = current_loss
...
if __name__ == '__main__':
    ...
    if args.resume:
        ...
        checkpoint = torch.load('./checkpoint/ckptRatio.t7')
        net = checkpoint['net']
