...
class EventDataset(Dataset):
    def __init__(self, jets, constituents, pca, targets,
...
class SimpleNet(nn.Module):
    def __init__(self, debug=False):
        ...
        self.linear1 = nn.Sequential(nn.Linear(200,50), nn.ReLU())
        self.linear2 = nn.Sequential(nn.Linear(9,50), nn.ReLU())
        self.relu = nn.ReLU()
        self.linear3 = nn.Sequential(nn.Linear(6,50), nn.ReLU())
        self.linear4 = nn.Linear(150,1)
        self.debug = debug
    def forward(self, constituents, jets, pca):
        if self.debug:
...
class ResolutionLoss(torch.nn.Module):
    ...
    def forward(self,x,y):
        resolution = (y-x)**2 - y # pay more attention to low mass events
        return torch.mean(resolution)
...
def train(epoch):
    ...
    for batch_idx, data in enumerate(train_loader):
        ...
        optimizer.zero_grad()
        jets, cons, pca, targets = Variable(jets.cuda()), Variable(cons.cuda()), Variable(pca.cuda()), Variable(targets.cuda())
...
def validate(epoch):
    ...
    if current_loss < best_loss:
        ...
        if not os.path.isdir('checkpoint'):
            os.mkdir('checkpoint')
        torch.save(state, './checkpoint/ckptMSESreluPCA.t7')
        best_loss = current_loss
...
if __name__ == '__main__':
    ...
    if args.resume:
        ...
        checkpoint = torch.load('./checkpoint/ckptMSESreluPCA.t7')
        net = checkpoint['net']
    ...
    if not args.distributed:
        if nGPUs > 1 and not args.resume:
